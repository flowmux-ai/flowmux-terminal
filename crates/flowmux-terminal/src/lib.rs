...
/// rcfile sourced by the default bash (`bash --rcfile <this>`). The default
/// shell is non-login on purpose — a real `bash -l` runs the user's login
/// profile, which on some setups (Ubuntu 22.04) resets `PATH` without
/// `/usr/bin` and hides base tools like `xset`. But running *no* profile at
/// all loses a `PS1`/env defined only in `~/.profile` / `~/.bash_profile`
/// (the prompt falls back to the system default). This script gets both: it
/// emulates a login shell's init (sources `/etc/profile` then the first
/// existing login profile), then re-appends the standard system dirs the
/// profile may have dropped — the same "append missing only" repair as
/// [`engine::ensure_standard_path`], expressed in shell so it runs *after*
/// the profile. Bash still auto-sources `/etc/bash.bashrc` before this file.
pub const DEFAULT_BASH_RCFILE: &str = r#"# Generated by flowmux. Do not edit; regenerated on every launch.
if [ -r /etc/profile ]; then . /etc/profile; fi
for __fm_f in "$HOME/.bash_profile" "$HOME/.bash_login" "$HOME/.profile"; do
    if [ -r "$__fm_f" ]; then . "$__fm_f"; break; fi
done
unset __fm_f
for __fm_d in /usr/local/sbin /usr/local/bin /usr/sbin /usr/bin /sbin /bin /usr/games /usr/local/games; do
    case ":$PATH:" in
        *":$__fm_d:"*) ;;
        *) PATH="$PATH:$__fm_d" ;;
    esac
...
/// Write [`DEFAULT_BASH_RCFILE`] to a stable per-user runtime path and return
/// it, for use as the default bash's `--rcfile`. Returns `None` if the file
/// cannot be written (caller falls back to a plain non-login shell). Writing
/// on every spawn is intentional: it keeps the script current if flowmux is
/// upgraded while the old GUI is still running.
pub fn write_default_bash_rcfile() -> Option<PathBuf> {
    let dir = std::env::var_os("XDG_RUNTIME_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(std::env::temp_dir);
    let path = dir.join("flowmux-shell-init.bash");
    std::fs::write(&path, DEFAULT_BASH_RCFILE).ok()?;
    Some(path)
}
...
mod tests {
    ...
    #[test]
    fn default_bash_rcfile_sources_login_profile_and_repairs_path() {
        let rc = DEFAULT_BASH_RCFILE;
        // Emulates login init: sources /etc/profile and a login profile.
        assert!(rc.contains(". /etc/profile"));
        assert!(rc.contains("$HOME/.profile"));
        assert!(rc.contains("$HOME/.bash_profile"));
        // Repairs PATH after the profile so /usr/bin can't be dropped, and
        // re-exports it. Every standard dir must be re-appended.
        assert!(rc.contains("export PATH"));
        for dir in super::engine::STANDARD_PATH_DIRS {
            assert!(rc.contains(dir), "rcfile must re-append {dir}");
        }
    }
